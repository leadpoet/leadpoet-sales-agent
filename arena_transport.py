...
def _result_data_any(payload: Any) -> Any:
    """The provider's result data whatever its type (a page body is a string)."""
...
def _canonical_linkedin_company_url(value: Any) -> str:
    """Return https://www.linkedin.com/company/<slug>/ for a LinkedIn company page, else ""."""
...
def _record_bucket(record: dict[str, Any]) -> str:
    ...
    if start is None or start < 0:
        return ""
    for floor, label in _LINKEDIN_BUCKETS:
        if start >= floor:
            return label
    return ""


def _record_matches_domain(record: dict[str, Any], domain: str) -> bool:
    observed = _domain(record.get("website"))
    return bool(observed) and (
        observed == domain
        or observed.endswith("." + domain)
        or domain.endswith("." + observed)
    )


def _record_headquarters(record: dict[str, Any]) -> str:
    locations = record.get("locations")
    if not isinstance(locations, list):
        return ""
    chosen = next(
        (item for item in locations if isinstance(item, dict) and item.get("headquarter")),
        next((item for item in locations if isinstance(item, dict)), None),
    )
    if not chosen:
        return ""
    parsed = chosen.get("parsed") if isinstance(chosen.get("parsed"), dict) else {}
    text = parsed.get("text") or ", ".join(
        str(part) for part in (chosen.get("city"), parsed.get("country") or chosen.get("country")) if part
    )
    return str(text or "").strip()[:200]


def _company_row_from_record(domain: str, record: dict[str, Any], linkedin_url: str) -> dict[str, Any]:
    """Project a LinkedIn company record into the stored-profile row shape."""

    industries = record.get("industries")
    founded = record.get("foundedOn")
    return {
        "normalized_domain": domain,
        "domain": domain,
        "company_name": str(record.get("name") or "").strip()[:200],
        "industry": str(industries[0]).strip()[:120] if isinstance(industries, list) and industries else "",
        "location": _record_headquarters(record),
        "linkedin_url": linkedin_url,
        "employee_count": record.get("employeeCount"),
        "year_founded": founded.get("year") if isinstance(founded, dict) else None,
        "updated_at": "",
        "source": "harvestapi_get_company",
    }


def _evidence_from_record(linkedin_url: str, record: dict[str, Any]) -> dict[str, str] | None:
    """The LinkedIn size band and HQ as the About-section projector reports them."""
    ...
    evidence = {
        "url": linkedin_url,
        "title": str(record.get("name") or "").strip()[:200],
        "employee_count": bucket,
        "quote": f"Company size {bucket} employees",
    }
    headquarters = _record_headquarters(record)
    if headquarters:
        evidence["listed_headquarters"] = headquarters
        evidence["headquarters_quote"] = f"Headquarters {headquarters}"
    return evidence
...
class ArenaToolClient:
    ...
    def __init__(self, timeout: float = 90.0, client: httpx.Client | None = None):
        ...
        # Set per run by the adapter from the ICP's contact and output policies.
        self.allow_contacts = False
        self.intent_details_policy: Any = None
        # Company LinkedIn pages seen during research, by website domain. The
        # contact pass searches by page URL; the output never carries them.
        self.linkedin_hints: dict[str, str] = {}
        # Verified LinkedIn company records by website domain, so the contact
        # pass need not fetch them again.
        self.company_records: dict[str, dict[str, Any]] = {}
        # Set by the adapter: look the company up through its LinkedIn record
        # (two cheap calls) before the 0.70-credit stored-profile query.
        self.structured_profile = False
    ...
    def fetch_homepage_html(self, arguments: Any) -> str:
        """Raw homepage HTML through Deepline's generic HTTP route.

        The judge binds a company's identity by finding its LinkedIn page
        linked from the live homepage. Miner code may not call Scrapingdog, so
        this is the brokered way to read that HTML; it spends one Deepline call.
        """

        raw = arguments.get("url") if isinstance(arguments, dict) else arguments
        raw = str(raw or "").strip()
        if raw.startswith("http://"):
            raw = "https://" + raw[len("http://"):]
        if not raw.startswith("https://"):
            raise ValueError("homepage URL must be https")
        payload = self._deepline("generic_http_request", {"url": raw, "method": "GET"})
        data = _result_data_any(payload)
        if isinstance(data, dict):
            for key in ("html", "body", "text", "content", "data"):
                if isinstance(data.get(key), str):
                    data = data[key]
                    break
        if not isinstance(data, str):
            raise RuntimeError("homepage fetch returned no page body")
        return data[:2_000_000]

    def _deepline(self, tool: str, payload: dict[str, Any]) -> dict[str, Any]:
        # One Deepline call per frame; the Arena quota is 30 per ICP attempt.
        self.deepline_calls = int(getattr(self, "deepline_calls", 0)) + 1
    ...
    def _contact_provider(self, tool: str, arguments: dict[str, Any]) -> dict[str, Any]:
        if tool == "harvestapi_get_profile":
            ...
            if set(arguments) != {"email"} or len(email) > 254 or not _EMAIL_RE.match(email):
                raise ValueError("email validation arguments are invalid")
            arguments = {"email": email}
        elif tool == "harvestapi_get_company":
            if set(arguments) == {"search"}:
                # Name lookup for a company whose page the research DB lacks.
                name = str(arguments.get("search") or "").strip()
                if not name or len(name) > 200:
                    raise ValueError("company record search is invalid")
                arguments = {"search": name}
            elif set(arguments) == {"url"}:
                company_url = _canonical_linkedin_company_url(arguments.get("url"))
                if not company_url:
                    raise ValueError("company record URL is invalid")
                arguments = {"url": company_url}
            else:
                raise ValueError("company record arguments are invalid")
            ...
            if "currentCompanies" in arguments:
                # HarvestAPI matches a company page only by its full URL or
                # numeric id; a bare "linkedin.com/company/x" returns nothing.
                company_url = _canonical_linkedin_company_url(arguments["currentCompanies"])
                if not company_url:
                    raise ValueError("contact search company URL is invalid")
                arguments = {**arguments, "currentCompanies": company_url}
    ...
    def _structured_company_record(self, domain: str) -> tuple[dict[str, Any], str] | None:
        """Find the company's LinkedIn page with Exa and read its record.

        Two cheap calls (0.13 credits) replace the 0.70-credit stored-profile
        query whenever the record's website is this domain; the record also
        carries the employee band the judge checks.
        """

        payload = self._deepline(
            "exa_search",
            {
                "query": f"{domain} LinkedIn company page",
                "numResults": 5,
        ...
        if _exa_reported_error(payload):
            return None
        results = _result_data(payload).get("results")
        candidates: list[str] = []
        for item in results if isinstance(results, list) else []:
            page = _canonical_linkedin_company_url(item.get("url") if isinstance(item, dict) else "")
            if page and page not in candidates:
                candidates.append(page)
        for page in candidates[:2]:
            record = _result_data_any(self._deepline("harvestapi_get_company", {"url": page}))
            element = record.get("element") if isinstance(record, dict) else None
            if isinstance(element, dict) and element and _record_matches_domain(element, domain):
                return element, _canonical_linkedin_company_url(element.get("linkedinUrl")) or page
        return None
    ...
    def get_company_profile(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            if self.structured_profile:
                structured = self._structured_company_record(domain)
        except Exception as exc:  # noqa: BLE001 - the stored profile remains the fallback
            _raise_profile_run_limit(exc)
            structured_errors.append(
                {"source": "linkedin_company_record", "error": f"record lookup failed: {type(exc).__name__}"}
            )
        if structured is not None:
            record, linkedin_url = structured
            self.linkedin_hints[domain] = linkedin_url
            self.company_records[domain] = dict(record)
            company = _company_row_from_record(domain, record, linkedin_url)
            _project_employee_count(company, company.get("employee_count"))
            include_financing = bool(arguments.get("include_financing"))
            financing = (
                self.get_company_events({"domain": domain, "categories": ["FUNDING"], "limit": 3})
            ...
            if not include_financing:
                profile["financing_lookup"] = (
                    "skipped; request include_financing only when no article you read "
                    "names the company's latest round"
                )
            evidence = _evidence_from_record(linkedin_url, record)
            if evidence is not None:
                profile["linkedin_profile_evidence"] = evidence
            return profile
        columns = (
        ...
        # The PredictLeads financing lookup costs about five searches and was
        # most of a round's Deepline spend. Run it only on request: a funding
        # article that names the round usually proves the stage already.
        include_financing = bool(arguments.get("include_financing"))
        if include_financing:
            ...
            financing = {"events": [], "errors": []}
        _project_employee_count(company, company.get("employee_count"))
        errors.extend(structured_errors)
        hint = _canonical_linkedin_company_url(company.get("linkedin_url"))
        if hint:
            self.linkedin_hints[domain] = hint
    ...
    def call(self, name: str, arguments: dict[str, Any]) -> Any:
        ...
        if name in _CONTACT_TOOLS:
            return self._contact_provider(name, arguments)
        if name == "fetch_homepage_html":
            return self.fetch_homepage_html(arguments)
