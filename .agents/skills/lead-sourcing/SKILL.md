---
name: lead-sourcing
description: Source evidence-backed companies with buying signals using local Deepline/ScrapingDog wrappers; for company and account lists, not people enrichment or outreach.
---
...
Preserve launcher-saved `original_text`; only users change criteria. Company
geography does not restrict activity location unless requested.
...
resolve flagged variants before acceptance.
`tyche_start` defaults: $0.80/company, no deadline. Set `max_duration_seconds` for user deadlines. Use `max_age_months` for calendar months or `max_age_days` for days.
...
confirm, evidenced rejection, or specifically justified hold.
...
2. **Qualify the company.**
   Review fit/signals. Snippets identify candidates; capture qualifying pages
   ...
   Observations do not establish duration/acceleration. Source company LinkedIn URLs; never invent slugs.
   ...
   [Harvest fields](references/output-contract.md#linkedin-company-size):
   companies require published employee range/source.
3. **Finalize and save.** Use `tyche_review` for changes.
   ...
   [Intent Details](references/output-contract.md#client-writing-and-taxonomy-version-20) together.
   Preserve valid descriptions.
   QA company fields, Signals and prose.
   ...
   Reconcile contrary findings; target negative-exclusion checks.
...
find another evidence route or company instead.
...
Sourcing authorizes scoped company research and enrichment.
...
`Companies`: one row per confirmed company. No person or communication fields.
...
- Evidence: [semantics](references/output-contract.md#semantic-checks), [attribution](references/output-contract.md#accepted-company-sources), [schema](references/output-contract.md#resultsjson-schema).
