...
def build_prompt(icp: dict[str, Any], max_companies: int | None = None) -> str:
    ...
    return (
        ...
        "- If the criterion names proof sources (press release, company announcement, product page, changelog) "
        "and not news, cite that source itself (the company's page or official release), never an article about it. "
        "A raise at a later stage than company_stage (Series B for Series A) disqualifies the candidate. "
        "Joining an accelerator, partner, perks, or marketplace program is not a strategic partnership.\n\n"
